...
var fg, og,mg,ag,tg,sg;
var score;
var ground,ground2,iground,grass,bI,sI;
...
function preload(){
  ...
  bI = loadImage("dry-tree-drawing-28.png")
   sI = loadImage("Strawberry.png")
}
...
background(23,165,137);
  if (gamestate === PLAY){
   ...
   if(keyDown("space")&&  monkey.y > 50) {
         monkey.velocityY = -(12+5*score/100);
}
...
if(fg.isTouching(monkey)){
  ...
  score = score+ 10;
    ...
    if(sg.isTouching(monkey)){
   sg.destroyEach();
  score = score+ 25;
   }
     if(og.isTouching(monkey)||tg.isTouching(monkey)){
   gamestate = END
     }
  if (ground.x < 0){
      ground.x = ground.width/2;
...
function spawntrees(){
  if(score >= 100){
 if (frameCount % 300 === 0){
    var tr = createSprite(400,300,10,40);
   tr.velocityX = -(8+5*score/200 ) 
   
    //generate random obstacles
     tr.y = Math.round(random(340,341));
     tr.addImage(bI) 
   tr.scale = 8
tr.scale = 0.1;
   tr.lifeTime = 34;
...
function spawnStrawberry() {
  if (score >=100){
  if (frameCount % 300 === 0) {
    var sb = createSprite(600,120,40,10);
    sb.y = Math.round(random(30,330));
    sb.addImage(sI);
   sb.scale = 0.02;
 sb.velocityX = -(8+1.5*score/100);
